...
export default function Queue(props) {
    ...
    const { state, send } = useContractFunction(rectangleGoverner, 'queue', { transactionName: 'VoteCast' });
    ...
    const [currentlyQueuingProposalId, setCurrentlyQueuingProposalId] = useState(undefined); //same as creating your state variable where "Next" is the default value for buttonText and setButtonText is the setter function for your state variable instead of setState

    useEffect(() => {
        if (state.status == 'Success') {
            notify("Thanks For Queuing!", `Queued ${currentlyQueuingProposalId}`);
            setTimeout(() => {
                setShowLoadingButton(false);
                setCurrentlyQueuingProposalId(undefined)
    ...
    const queue = () => async (event) => {
        ...
        setCurrentlyQueuingProposalId(proposalId)
    }
        ...
        <>
            {
                showLoadingButton ?
                    ...
                    <>
                        ...
                        <button onClick={queue()} className="btn btn-primary bg-success border-success hover:bg-success mr-2">Queue</button>
                    </>
            }
